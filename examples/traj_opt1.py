...
x0s = [
    jnp.array([[-0.1], [-0.1], [jnp.pi / 3], [0.02 ]]),
    jnp.array([[-0.1], [-0.1], [jnp.pi / 6], [0.02 ]]),
    jnp.array([[-0.2], [-0.1], [jnp.pi / 6], [0.02 ]]),
    jnp.array([[-0.2], [-0.1], [0.0 / 3], [0.02 ]])
]
...
for target_index in range(len(x0s)):
    ...
    for rep_index in range(N_repetition):
        u_opt, opt_time, final_loss = ic_traj_opt(
            ps, x0, jnp.array([[0.0],[0.0],[0.0],[0.0]]),
            N=N_points, N_traj_opt=10, dt=dt, u_step_size=u_step_size
        )
...
plt.savefig("../data/traj_opt1/simulate1-u_01-xy.png")
...
plt.ylabel('s_x')
plt.title('Trajectory With Constant u=[0.1;0] input')
plt.savefig("../data/traj_opt1/simulate1-u_01-t_x.png")
...
plt.xlabel('t')
plt.ylabel('p_y')
plt.savefig("../data/traj_opt1/simulate1-u_01-t_p_y.png")
